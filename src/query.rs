...
use crate::{
    Database, Result, FilterOperator, Aggregate, Operator, Value, Pagination, PaginatedResult,
    Sort
};
...
impl QueryBuilder {
    ...
    /// Add a join clause
    pub fn join(mut self, join_type: crate::JoinType, table: impl Into<String>, condition: impl Into<String>) -> Self {
    ...
    pub fn join_as(mut self, join_type: crate::JoinType, table: impl Into<String>, alias: impl Into<String>, condition: impl Into<String>) -> Self {
        self.joins.push(JoinClause {
            join_type,
            table: table.into(),
    ...
    pub fn aggregate(mut self, function: Aggregate, column: impl Into<String>, alias: Option<impl Into<String>>) -> Self {
    ...
    /// Add where condition
    pub fn where_condition(mut self, condition: &str, _params: impl Into<Vec<libsql::Value>>) -> Self {
        // This is a simplified implementation - in a real implementation you'd parse the condition
        self.where_clauses.push(FilterOperator::Custom(condition.to_string()));
        self
    }

    /// Add search
    pub fn search(mut self, field: &str, query: &str) -> Self {
        let condition = format!("{} LIKE '%{}%'", field, query);
    ...
    pub fn having_condition(mut self, condition: &str, _params: impl Into<Vec<libsql::Value>>) -> Self {
        // This is a simplified implementation
        self.having.push(FilterOperator::Custom(condition.to_string()));
        self
    }
    ...
    pub fn where_in(mut self, field: &str, subquery: QueryBuilder) -> Self {
        ...
        let condition = format!("{} IN ({})", field, subquery_sql);
        self.where_clauses.push(FilterOperator::Custom(condition));
        self
    }
    ...
    fn build_where_clause(&self, filters: &[FilterOperator]) -> Result<(String, Vec<libsql::Value>)> {
        let mut sql = String::new();
        let mut params = Vec::new();
    ...
    fn build_filter_operator(&self, filter: &FilterOperator) -> Result<(String, Vec<libsql::Value>)> {
        match filter {
            ...
            FilterOperator::Custom(condition) => {
                Ok((condition.clone(), vec![]))
            }
        }
    }
        ...
        while let Some(row) = rows.next().await? {
            ...
            for i in 0..row.column_count() {
                if let Some(column_name) = row.column_name(i) {
                    ...
                    map.insert(column_name.to_string(), self.libsql_value_to_json_value(&value));
                }
            }
    ...
    {
        // Get total count
        let count_builder = QueryBuilder::new(&self.table)
            .select(vec!["COUNT(*) as count"]);
