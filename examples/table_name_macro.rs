...
/// to specify custom table names for your models in a Cloudflare Worker environment.

use worker::*;
use libsql_orm::{Model, Database, generate_migration, MigrationManager};
...
async fn main(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    ...
    println!("User struct with #[table_name(\"user_accounts\")] -> table: '{}'", User::table_name());
    println!("BlogPost struct with #[table_name(\"blog_posts\")] -> table: '{}'", BlogPost::table_name());
    println!("Customer struct with #[table_name(\"legacy_customer_data\")] -> table: '{}'", Customer::table_name());
    ...
    println!("✓ Created product in '{}' table: {}", Product::table_name(), created_product.name);
    ...
    println!("✓ Created user in '{}' table: {}", User::table_name(), created_user.username);
    ...
    println!("✓ Created blog post in '{}' table: {}", BlogPost::table_name(), created_post.title);
    ...
    println!("✓ Created customer in '{}' table: {}", Customer::table_name(), created_customer.company_name);
    
    // Query data from custom tables
    println!("\n4. Querying Data from Custom Tables:");
    
    let products = Product::find_all(db).await?;
    println!("Found {} products in '{}' table", products.len(), Product::table_name());
    
    let users = User::find_all(db).await?;
    println!("Found {} users in '{}' table", users.len(), User::table_name());
    
    let posts = BlogPost::find_all(db).await?;
    println!("Found {} blog posts in '{}' table", posts.len(), BlogPost::table_name());
    
    let customers = Customer::find_all(db).await?;
    println!("Found {} customers in '{}' table", customers.len(), Customer::table_name());
    ...
    Response::ok("✅ Table name macro example completed successfully!")
}