...
use worker::*;
use libsql_orm::{Model, Database, MigrationManager, generate_migration, FilterOperator, Filter, Value, Pagination};
...
async fn fetch(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    ...
    match (req.method(), req.path().as_str()) {
        ...
        (Method::Get, "/posts") => {
            let url = req.url()?;
            let page: usize = url.query_pairs()
                .find(|(key, _)| key == "page")
                .and_then(|(_, value)| value.parse().ok())
                .unwrap_or(1);
            let per_page: usize = url.query_pairs()
        ...
        (Method::Get, "/posts/published") => {
            match Post::find_where(
                FilterOperator::Single(Filter::eq("published", true)),
                &db
            ).await {
        ...
        // Health check
        (Method::Get, "/health") => {
            Response::ok("libsql-orm Cloudflare Worker is healthy!")
        }
