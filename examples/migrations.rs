...
use libsql_orm::{
    generate_migration, templates, Database, MigrationBuilder, MigrationManager, Model,
};
...
/// database schema changes over time in a standalone application.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_url = env::var("LIBSQL_DATABASE_URL")
        .unwrap_or_else(|_| "file:migrations_example.db".to_string());
    ...
    let posts_migration = templates::create_table(
        "posts",
        &[
    ...
    let mut rows = manager
        ...
        .query(
            query_sql,
            vec![libsql::Value::Text("john.doe@example.com".to_string())],
        )
        .await?;
