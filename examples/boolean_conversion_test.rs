...
use libsql_orm::{generate_migration, Database, Filter, FilterOperator, MigrationManager, Model};
...
/// boolean values (0/1) are properly converted to Rust boolean types in a standalone application.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_url =
        env::var("LIBSQL_DATABASE_URL").unwrap_or_else(|_| "file:test.db".to_string());
    ...
    for user in &created_users {
        println!("User: {} - Active: {}, Verified: {}, Premium: {}, Can Edit: {}, Enabled: {}, Published: {}", 
            user.name, user.is_active, user.is_verified, user.has_premium, user.can_edit, user.enabled, user.published);
    }
    ...
    for user in &queried_users {
        ...
        println!(
            "  is_active: {} (type: {})",
            user.is_active,
            std::any::type_name_of_val(&user.is_active)
        );
        println!(
            "  is_verified: {} (type: {})",
            user.is_verified,
            std::any::type_name_of_val(&user.is_verified)
        );
        println!(
            "  has_premium: {} (type: {})",
            user.has_premium,
            std::any::type_name_of_val(&user.has_premium)
        );
        println!(
            "  can_edit: {} (type: {})",
            user.can_edit,
            std::any::type_name_of_val(&user.can_edit)
        );
        println!(
            "  enabled: {} (type: {})",
            user.enabled,
            std::any::type_name_of_val(&user.enabled)
        );
        println!(
            "  published: {} (type: {})",
            user.published,
            std::any::type_name_of_val(&user.published)
        );
        println!(
            "  age: {} (type: {})",
            user.age,
            std::any::type_name_of_val(&user.age)
        );
        println!(
            "  score: {} (type: {})",
            user.score,
            std::any::type_name_of_val(&user.score)
        );
        println!();
    }
    ...
    let active_users =
        TestUser::find_where(FilterOperator::Single(Filter::eq("is_active", true)), db).await?;
    println!("✓ Found {} active users", active_users.len());

    let verified_users =
        TestUser::find_where(FilterOperator::Single(Filter::eq("is_verified", true)), db).await?;
    println!("✓ Found {} verified users", verified_users.len());

    let premium_users =
        TestUser::find_where(FilterOperator::Single(Filter::eq("has_premium", true)), db).await?;
    ...
    println!(
        "✓ Found {} users that are active but not verified",
        filtered_users.len()
    );

    // Verify serialization/deserialization works
    println!("\n6. Testing JSON serialization/deserialization:");
    if let Some(user) = queried_users.first() {
