...
use quote::quote;
use syn::{parse_macro_input, DeriveInput, Data, Fields, Field, Lit, Type, Attribute};
...
pub fn derive_model(input: TokenStream) -> TokenStream {
    ...
    let (field_names, column_definitions, boolean_field_names, boolean_flags) = if let Data::Struct(data) = &input.data {
...
fn parse_column_definition(field: &Field) -> proc_macro2::TokenStream {
    ...
    let default_def = match &field.ty {
        Type::Path(type_path) => {
            ...
            match type_name.to_string().as_str() {
                "i64" => format!("{} INTEGER", field_name_str),
                "i32" => format!("{} INTEGER", field_name_str),
                "f64" => format!("{} REAL", field_name_str),
                "f32" => format!("{} REAL", field_name_str),
                "bool" => format!("{} BOOLEAN", field_name_str),
                "String" => format!("{} TEXT", field_name_str),
                _ => format!("{} TEXT", field_name_str),
            }
        }
        _ => format!("{} TEXT", field_name_str),
    };
    ...
    for attr in &field.attrs {
        if attr.path().is_ident("orm_column") {
            ...
            if primary_key {
                column_def = format!("{} PRIMARY KEY", column_def);
            }
            if auto_increment {
                column_def = format!("{} AUTOINCREMENT", column_def);
            }
            if not_null {
                column_def = format!("{} NOT NULL", column_def);
            }
            if unique {
                column_def = format!("{} UNIQUE", column_def);
            }
